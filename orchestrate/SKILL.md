...
2. Before the first dispatch, write a dispatch plan in the session: one line per track naming its owner (Codex, Opus, orchestrator), the tracks that run concurrently, and the data dependency that forces any serialization. Re-issue the plan whenever a new track is added. Parallelize independent work. Serialize only on true data dependencies; never idle on one delegate while dispatchable work exists, and advance other tracks while a gate blocks one. Isolate concurrent writers; a reviewer must see stable source. Read only the applicable procedure in [references/pipelines.md](references/pipelines.md).
