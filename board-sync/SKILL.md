...
3. **Reconcile existing state.** `gh search issues --repo <repo> "board-sync:<project-key> in:body"` (fallback: `gh issue list --search`). Anything already marked gets `gh issue edit`, not re-creation. Done when: every slice is mapped to create-or-update.
